...
//    }
    ...
    virtual bool lineOfSight(const NodeId n1, const NodeId n2) const override
    {
        Vectori p1 = idToPos(n1);
        Vectori p2 = idToPos(n2);

        auto diff = p2 - p1;

        Vectorf normal = {std::abs(diff.x), std::abs(diff.y)};

        Vectori sign = {diff.x > 0 ? 1 : -1, diff.y > 0 ? 1 : -1};

//        auto p = new Point(p0.x, p0.y);
//        var points = [new Point(p.x, p.y)];
        for(Vectori i; i.x < normal.x || i.y < normal.y;)
        {
//            if ((0.5+ix) / nx < (0.5+iy) / ny)
            if((i.x + 0.5) / normal.x < (i.y + 0.5) / normal.y)
            {
...
private:
    ...
    //But I find this way make the adaptor way more reusable
     std::function<bool(const Vectori&)> mIsTraversable;
};
